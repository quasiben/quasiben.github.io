...
if $NIC_BINDING; then
   UCX_STRING="UCX_NET_DEVICES=$UCX_NET_DEVICES"
else
   UCX_STRING="<UCX binding disabled, set NIC_BINDING=true>"
fi

echo "MPI Rank $OMPI_COMM_WORLD_RANK host $(hostname) GPU=$CUDA_VISIBLE_DEVICES cores=$cores membind=$MEMLIST $UCX_STRING"
